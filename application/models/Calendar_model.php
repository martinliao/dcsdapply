...
class Calendar_model extends MY_Model
{
    ...
    public function apply($userID, $calendarID)
    {
            ...
            if (is_array($tmp) && count($tmp) > 0) {
                ...
                if ($ssTime <= $ccTime && $ccTime <= $eeTime) {
                    ...
                    $sql = "SELECT 
                            ...
                            where volunteer_calendar.date >= '" . $sss[0] . "' 
                              and volunteer_calendar.start_time >= '" . $sss[1] . "' 
                              and volunteer_calendar.date <= '" . $eee[0] . "' 
                              and volunteer_calendar.end_time <= '" . $eee[1] . "' 
                              and volunteer_calendar_apply.userID = '" . $userID . "'
                              and volunteer_classroom.volunteerID = '" . $volunteerID . "'
                              " . $long_range_where . "
        ...
        // 4.不同教室 
        // 看是否有資料
        $sql = "SELECT * FROM `volunteer_calendar` 
                  WHERE `date` = '" . $calendar_data->date . "' 
                    AND `type` = '" . $type . "' 
                    AND `courseID` = '" . $calendar_data->courseID . "' 
                    AND `vcID` != '" . $calendar_data->vcID . "' ";
